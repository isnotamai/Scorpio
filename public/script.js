(function() {
  ...
  function updateAuthUI() {
    ...
    if (user && token) {
      ...
      badge.className = 'auth-role-badge role-' + (user.role || 'user');
      // Fetch quota
      authFetch('/api/me').then(r => r.json()).then(data => {
        if (data.quota) {
          const quotaText = data.quota.max === -1 ? data.quota.used + '/\u221E' : data.quota.used + '/' + data.quota.max;
          document.getElementById('auth-quota').textContent = quotaText;
        }
        // Update stored user with latest role
  ...
  function updateUploadPanel() {
    ...
    const usernameDisplay = document.getElementById('upload-username-display');
    if (!keyField) return;
    if (token && user) {
      keyField.style.display = 'none';
      userInfo.style.display = '';
      usernameDisplay.textContent = 'Uploading as ' + user.username;
    } else {
  ...
  function openModal(mode) {
    ...
    document.getElementById('modal-toggle-text').textContent = mode === 'login' ? "Don't have an account? " : 'Already have an account? ';
    document.getElementById('modal-toggle-link').textContent = mode === 'login' ? 'Register' : 'Login';
    document.getElementById('auth-status').classList.remove('is-visible');
  ...
  document.getElementById('auth-submit').addEventListener('click', async () => {
    ...
    try {
      ...
      updateAuthUI();
      if (data.api_key) {
        showToast('Registered! Your API key has been generated.');
      } else {
        showToast('Logged in!');
      }
    } catch (err) {
  ...
  document.querySelectorAll('.tool-card').forEach((card) => {
    card.addEventListener('click', () => openPanel('panel-' + card.dataset.tool));
  });

  document.querySelectorAll('[data-back]').forEach((btn) => {
    btn.addEventListener('click', () => {
      const openPanelEl = document.querySelector('.tool-panel.is-open');
      if (openPanelEl) {
        openPanelEl.style.opacity = '0';
        openPanelEl.style.transform = 'translateY(-8px)';
        openPanelEl.style.transition = 'opacity .15s, transform .15s';
        setTimeout(() => {
          openPanelEl.style.opacity = '';
          openPanelEl.style.transform = '';
          openPanelEl.style.transition = '';
          document.querySelectorAll('.tool-panel').forEach((p) => p.classList.remove('is-open'));
  ...
  async function handleUpload(file) {
    ...
    if (token) {
      ...
      if (!apiKey) {
        showStatus(uploadStatus, 'Please enter an API key or log in', false);
        return;
      }
    ...
    try {
      const res = await fetch('/upload', {method: 'POST', headers: uploadHeaders, body: formData});
      const data = await res.json();
